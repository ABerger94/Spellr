...
import { searchCards, searchTokenCards } from '@/server/scryfall/cardService';
...
export async function GET(req: Request) {
  ...
  const tokensOnly = searchParams.get('tokens') === '1';
  ...
  try {
    const result = tokensOnly ? await searchTokenCards(q, page) : await searchCards(q, page);
